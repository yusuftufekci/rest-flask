...
while True:
    ...
    if (datetime.datetime.now().second % 5 == 0) or (personNumber > personNumberTempSql + 10):
        ...
        val = (date_and_time, personNumber, "Yemekhane")
        mycursor.execute(sql, val)
        personNumberTempSql = personNumber
        mydb.commit()
        cv2.imwrite("/Users/yusuftufekci/Desktop/Yolo/yoloProject/snapshots/snapshot2.jpeg", frame)
        time.sleep(0.2)

    personNumber = 0
    cv2.imshow("Yemekhane", image)
